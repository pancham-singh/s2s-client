...
interface CreateAssessmentAnswerInput {
    body: string;
    question: string;
    isCorrect: boolean;
    id: string;
...
const createAssessmentAnswer = async (_, args: CreateAssessmentAnswerInput, { user }: GqlContext) => {
    if (!user) {
    ...
    try {
        ...
        const newAnswer = await AssessmentAnswer.query()
            .skipUndefined()
            .first()
            .insertGraphAndFetch(
                [
...
export default createAssessmentAnswer;
